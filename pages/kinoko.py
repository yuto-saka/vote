...
# コメントを保存するCSVファイルのパス
comments_file_path = "snack_comments.csv"
...
with col1:
    st.image("kinoko.jpg", caption="きのこの山", use_column_width=True)

with col2:
    st.image("takenoko.jpg", caption="たけのこの里", use_column_width=True)
...
# 投票ボタン
kinoko = st.button("きのこの山")
takenoko = st.button("たけのこの里")

if kinoko or takenoko:
    if st.session_state.vote:
        st.warning("すでに投票済みです！")
    else:
        if kinoko:
            st.session_state.votes = "きのこの山"
        else:
            st.session_state.votes = "たけのこの里"
        # 新しい投票をDataFrameに追加
...
if st.button("コメントを送信"):
    if user_comment.strip():  # 空白を除いたコメントが存在する場合
        if st.session_state.votes == "きのこの山" or st.session_state.votes == "たけのこの里":
